...
async function fetchAll({ persistSnapshot = true } = {}) {
  try {
    ...
    if (persistSnapshot) persistReachrLocalSnapshot(data);
    return data;
  } catch (e) {
    return cachedData;
  }
...
function isValidPostingIdentity(identity) {
  ...
  if (/^\d+\s*(?:m|h|d|w|mo|y)$/i.test(name)) return false;
  if (/^(facebook|facebook menu|meta|pages?|profiles?|home|watch|marketplace|groups?|notifications?|menu|account controls(?: and settings)?|account|your)$/i.test(name)) return false;
  if (/\b(number of unread notifications|new notification|notifications?|unread chats?|chat history is missing|available voices|privacy shortcuts|professional dashboard|ad center|create post|composer|search facebook|view all|sponsored|contacts|meta ai|profile photo|profile picture|online status indicator)\b/i.test(name)) return false;
  if (/^https?:\/\//i.test(name)) return false;
  const url = identity?.url || '';
  if (/(\/settings|\/help|\/privacy|\/policies|\/business|\/ads|\/ad_|\/groups\/|\/marketplace|\/events|\/friends|\/messages|\/notifications|\/stories\/)/i.test(url)) return false;
  return true;
}
...
function analyzeReachrHealth(posts = [], healthJobs = []) {
  ...
  [...byKey.values()].filter(rows => rows.length > 1).forEach(rows => {
    ...
    issues.push({
      ...
      level: 'warn',
      title: 'Possible duplicate campaigns',
      detail: `${rows.length} similar campaigns for ${scheduledEventIdentityName(keep) || 'one actor'}; review their IDs, comments, and settings before deleting any.`,
      autoFix: false,
      keepId: keep.id,
  ...
  enabledPosts.forEach(post => {
    ...
    if (hasSchedule && !endsAt && (maxRuns == null || maxRuns === '')) {
      issues.push({
        ...
        level: 'warn',
        title: 'No stop date',
        detail: `${reachrHealthCampaignName(post)} has no end date or run cap; confirm that indefinite scheduling is intended.`,
        autoFix: false,
        postId: post.id
      });
    }
    if (!scheduledEventIdentityName(post) || !isValidPostingIdentity({ name: scheduledEventIdentityName(post) })) {
      issues.push({
        ...
        detail: `${reachrHealthCampaignName(post)} has no valid Facebook profile/Page; review the posting actor before running it. No changes have been made.`,
        autoFix: false,
        postId: post.id
      });
    }
    if (reachrHealthGroupUrls(post).length === 0) {
      issues.push({
        ...
        detail: `${reachrHealthCampaignName(post)} has no groups selected; review its targets. No changes have been made.`,
        autoFix: false,
        postId: post.id
      });
    }
  ...
  if (repeatRows.length) {
    issues.push({
      ...
      detail: `${repeatRows.length} queued/processing row${repeatRows.length === 1 ? '' : 's'} can requeue independently; verify scheduling ownership before changing repeat settings.`,
      autoFix: false,
...
function renderReachrHealthPanel(health, fixes = []) {
  ...
  const label = worst === 'ok' ? 'Healthy' : 'Needs review';
  ...
  el.innerHTML = `<div class="reachr-health-card">
    <div class="reachr-health-top">
      <div><div class="reachr-health-title">Campaign health check</div><div class="reachr-health-subtitle">Read-only inspection when Upcoming loads. Campaigns and queued jobs are not changed. Review findings and use explicit campaign controls to make changes.</div></div>
...
async function loadScheduled() {
  // Inspection must not change persisted campaigns, jobs, or local snapshots.
  cachedData = await fetchAll({ persistSnapshot: false });
